...
object PutNode {
  private val putIdOffset = Node.nodeOffset
  ...
  def create
      (putId: Int,
       ...
       currentModId2: ModId): Pointer = {
    val size = 4
    val ptr = Node.create(size, Node.PutNodeType, currentModId1, currentModId2)

    MemoryAllocator.putInt(ptr + putIdOffset, putId)

    ptr
  }

  def getPutId(ptr: Pointer): Int = {
    MemoryAllocator.getInt(ptr + putIdOffset)
  }
}
