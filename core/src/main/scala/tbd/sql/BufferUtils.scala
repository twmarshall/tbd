...
object BufferUtils {
  /*
   * print the output table of an operator in format
   */
  def printOperator (oper: Operator) {
    oper.getAdjustable.toBuffer(oper.getTable.mutator).
    map(_._2.map(BufferUtils.getValue)).foreach { row => {
      row.foreach { datum => print(datum + "\t| ")
        }
      println
    }}
  }
