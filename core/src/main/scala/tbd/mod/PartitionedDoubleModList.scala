...
class PartitionedDoubleModList[T](
  ...
  def insert(mod: Mod[Any], respondTo: ActorRef, datastore: Datastore): Int = {
    var outerNode = datastore.tables("mods")(lists.id.value)
      .asInstanceOf[DoubleModList[Mod[DoubleModList[Any]]]]

    var lastMod: Mod[DoubleModList[Any]] = null
    while (outerNode != null) {
      val modList = datastore.tables("mods")(outerNode.value.id.value)
        .asInstanceOf[Mod[DoubleModList[Any]]]
      var innerNode = datastore.tables("mods")(modList.id.value)
        .asInstanceOf[DoubleModList[Any]]

      while (innerNode != null) {
        lastMod = innerNode.next
        innerNode = datastore.tables("mods")(innerNode.next.id.value)
          .asInstanceOf[DoubleModList[Any]]
      }
      outerNode = datastore.tables("mods")(outerNode.next.id.value)
        .asInstanceOf[DoubleModList[Mod[DoubleModList[Any]]]]
    }

    val newTail = datastore.createMod[DoubleModList[Any]](null)
    datastore.updateMod(lastMod.id, new DoubleModList(mod, newTail), respondTo)
  }
        ...
        if (innerNode.value == toRemove) {
          if (previousNode != null) {
            count += datastore.updateMod(previousNode.next.id,
                                         datastore.tables("mods")(innerNode.next.id.value),
                                         respondTo)
          } else {
          ...
          datastore.tables("mods") -= innerNode.next.id.value
	  datastore.tables("mods") -= innerNode.value.id.value
